...
import convictedAudDataSource from "../../../../devextreme/data-sources/audition/convicted-aud-data-source";
...
const onExporting = (e: ExportingEvent) => {
  ...
  const worksheet = workbook.addWorksheet(
    `История изменений заявлений на ${new Date().toDateString()}`
  );
  ...
  exportDataGrid({
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      saveAs(
        ...
        `История изменений заявлений на ${new Date().toDateString()}.xlsx`
      );
    });
  });
};

const ConvictedAudDataGrid = () => {
  ...
  const dataSource = React.useMemo(
    () => convictedAudDataSource(backendSettings),
    [backendSettings]
  );
  ...
  const onDataErrorOccured = React.useCallback(
    (
      e: EventInfo<dxDataGrid<ConvictedAudEntity, number>> &
        DataErrorOccurredInfo
    ) => {
      dispatch(
        addNotification({
...
export default ConvictedAudDataGrid;
