 ...
 */
const callTryingsDataSource = (backendSettings: BackendSettings) =>
  new CustomStore<CallTryingEntity, number>({
    key: 'id',
    ...
    load: async (loadOptions: any) => {
      ...
      try {
        const response = await fetchAllQuery<CallTryingEntity>(
          backendSettings,
          CALL_TRYINGS,
          params,
        );
    ...
    byKey: async (key) => {
      return await fetchOneQuery<CallTryingEntity>(
        backendSettings,
        CALL_TRYINGS,
        key,
      ).catch((err) => {
    ...
    remove: async (key) => {
      return await deleteQuery(backendSettings, CALL_TRYINGS, key).catch(
        (err) => {
          throw err;
        },
      );
    ...
    insert: async (values) => {
      return await insertQuery<CallTryingEntity>(
        backendSettings,
        CALL_TRYINGS,
        values,
      )
        .then((res) => {
          ...
          } else return res.data as CallTryingEntity;
        })
        .catch((err: Error) => {
          throw err;
        });
    ...
    update: async (key, values) => {
      ...
      await fetchOneQuery<CallTryingEntity>(backendSettings, CALL_TRYINGS, key)
        .then(async (res) => {
          // Then we should update changed values in fetched entity
          const returnedData: CallTryingEntity =
            updateValuesOfEntityDataTypeObject(res, values);
          // Then just update entity
          await updateQuery<CallTryingEntity>(
            backendSettings,
            CALL_TRYINGS,
            returnedData,
          )
...
export default callTryingsDataSource;
